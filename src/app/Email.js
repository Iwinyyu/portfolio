...
export default function Email() {
  ...
  return (
    <div ref={ref} id="contact" className={`transition delay-500 ${inView? " intersect":""} flex flex-col justify-center items-center gap-5 max-w-[450px] py-36`}>
      ...
      <div className="l5 transition ease-out cursor-pointer text-yellow-color capitalize bg-yellow-color rounded-md">
        <button onClick={openemail} className="font-Space text-sm transition duration-300 ease-in-out bg-dark-color hover:-translate-y-1 hover:-translate-x-1.5 py-4 px-5 rounded-md border-solid border-2 border-yellow-color">
          Say hello
        </button>
      </div>
    </div>
